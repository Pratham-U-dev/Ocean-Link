...
Follow these steps to demonstrate the system to judges:

1. **Verify the Normal State**:
   * Observe the dashboard. You will see 10 blue markers (boats) drifting slowly along the coast of Karnataka.
   * Small, dashed lines between nodes represent active LoRa connections (the topology graph).
   * Note the status bar showing the connected boats and "Gateway Connected" status.

2. **Trigger a Distress Event**:
   * Scroll down to the **SimControls** panel on the bottom-right.
   * Select a distant boat (e.g., `BOAT_07` near Bhatkal or `BOAT_05` near Udupi).
   * Click **"Trigger Distress"**.
   * **Watch and Listen**:
     1. An audible alarm beep will sound instantly.
     2. A red pulsing sonar ring will appear at the boat's location.
     3. You will see a yellow dashed line trace from the distress source, hopping from boat to boat, propagating down the coastline until it finally hits the **Shore Gateway (Mangaluru Port)**.
     4. A new high-priority alert card will appear in the **Alert Panel** with a red "ACTIVE" status badge.
     5. The alert card will show the detailed hop-by-hop route (e.g., `BOAT_07 -> BOAT_05 -> BOAT_03 -> BOAT_01 -> GATEWAY`).

3. **Coordinate the Response (Status Workflow)**:
   * In the control panel, click **"Acknowledge"** on the active alert. The status updates to **ACKNOWLEDGED**.
   * Note that an ACK packet is reverse-flooded back to the boat, showing the status update.
   * Click **"Dispatch Coast Guard"**. The status changes to **DISPATCHED** (amber color).
   * Once resolved, click **"Mark Resolved"** to clear the visual alarm state of the boat. The status becomes **RESOLVED** (green color).

4. **Reset the Simulation**:
   * Click the **"Reset All"** button in SimControls to clear all alerts and reset the boats to their initial coordinate positions for the next demo cycle.
